...
export function AssetAlertsComponent() {  

const ractiveData = {
    isolated: true,
    decorators: { tooltip: RactiveTooltip.default },
    template: `
        <div class="alert-cell alert-{{timeRemain||gridData.disconnected ? 'error' : regulation ? 'danger' : ''}}">
            {{#if disconnected}}
                <span 
            ...
            {{#if gridData.disconnected}}
                ...
                {{/if}}
                {{#if (gridData.value === 0 || gridData.value === 3) }}     
                    <span 
                        decorator='tooltip: Unknown status of grid connection'
                        class="icon dripicons-warning"> 
                    </span>
                {{/if}}
            {{/if}}
    ...
    data: {            
        'disconnected': false,
        'gridData': gridEvent(false,undefined),
        'regulation': false,
        'timeRemain': false,
        getSetpointMotive        
    },
...
function gridEvent (disconnected,signal) {
    let value = signal ? signal.Value:undefined;
    return {disconnected: disconnected, value: value };
}
    ...
    unsubscribes.push(
        resources.subscribeToAssets(assets => {
            ...
            ractive.set({
                ...
                gridData: gridEvent(gridConnection !== 2,asset.signals['GridConnection']),
